...
The VerifyEval function is the counter-part to CreateWitness. It's purpose is to check that a single point $(i, \phi(i))$ is indeed on the polynomial to which $C$ is a commitment.
...
Fleischhacker et al. use a so-called extractable witness key encapsulating mechansim to generate the keys for Alice and Bob. What is that? The main idea is very simple. 
Basically you create a scheme for a NP relation, where you derive a key for a statement and another party can can derive the same key if and only if they poses the witness to your NP statement. The NP relation in the case of this work is the realtion of opening points to a polynomial commitment as a statement and KZG witnesses as a witness. 

Let's take a a look at the concrete scheme: 
...
<!--As one opening represents only one choice, this is of course repeated for every $n$ chocies Bob has to make. The accumulated key is then a hash of the n computed $s$. -->

For any other party to obtain the same key, they have to receive the random $r$ of course somehow. However, sharing it in plain would lead to the same porblem as before, any party could derive the key from the statement. Thats why $r$ is send in the form of $(g^\alpha/g^i)^r$. This is hiding $r$ as deriving $r$ would exactly be solving the DL problem for $g^\alpha/g^i$. And it allows the other party to compute the key as $k':=e(\omega, (g^\alpha/g^i)^r)$, where $\omega$ is a KZG witness. The correctness for this stems from the VerifyEval equations correctness: 
...
\end{aligned}
```
Note the 3rd equivalence holds because r is uniform random and thus with non-negligible probabillity non zero. 

Here is a graphic of the paper summarizing the scheme. Note the paper uses additive notaiton for group operations. Additionally the paper displays a
verison where $\alpha$ and $\beta$ are not single values, but vectors of values. We will not need this for the laconic OT construction.
...
The basic Laconic OT constrcution is a simple combination of the two previous sections, the WKEM and the hiding KZG vector commitment scheme. 
...
```math
    k_0:= Encap(SRS, C, i, 0) \land  k_1:= Encap(SRS, C, i, 0)
```
and computes the accoding ciphertexts 
```math
...
The security follows from the hiding KZG vector commitment scheme and the WKEM. Note particularly recomputing a key not accoding to Bob's choice is equivlent to breaking the KZG's binding property. 

## Benchmarks
The paper provides benchmarks from an open-soruce implementation which can be found [here](https://github.com/rot256/research-we-kzg).
The implementation uses the BLS12-381 elliptic curve for the pairing friendly group for the KZG and is implemented in Rust using arkworks. The benchmarks are performed on a i7-11800H @ 2.30GHz CPU with 64 GB of RAM. The Benchmarks are performed on 256-bit messages.
Here is a graphic of the benchmarks from the paper.
![graphic of benchmarks](benchmarks.png)
D is the vector of the choices of Bob i.e. how many messages are being send and received, pp is the SRS, and the digest is the KZG commitment. The Hash is essentially the Setup of the Laconic OT.

## Sum up

We just saw how Laconic i.e batched OT can be constructed using the KZG. Bob commits to a vector of his choices _D_ using the KZG. Alice then uses an key encapsultion mechanism based on Bob's commitment to create two keys, with which she encrypts two messages. Bob is bound by the commitment to his choice and can only recompute one of the keys and hence only one message, the message of his choice.

Note that the Laconic OT outlined is a 1 of 2 OT version. However, since the i-th choice is commited as the evaluation of the polynomial at i, the construction could be just as well extended to a 1 of n version.